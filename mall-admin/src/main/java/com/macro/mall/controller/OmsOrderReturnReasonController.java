...
public class OmsOrderReturnReasonController {
    ...
    @ResponseBody
    public CommonResult create(@RequestBody OmsOrderReturnReason returnReason, Principal principal) {
        if(principal==null){
            return CommonResult.unauthorized(null);
        }
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        returnReason.setAdminId(umsAdmin.getId());
    ...
    public CommonResult<CommonPage<OmsOrderReturnReason>> list(@RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize,
        ...
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        List<OmsOrderReturnReason> reasonList = orderReturnReasonService.list(umsAdmin.getId(),pageSize, pageNum);
