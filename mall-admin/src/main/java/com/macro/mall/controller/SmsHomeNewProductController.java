...
public class SmsHomeNewProductController {
    ...
    @ResponseBody
    public CommonResult create(@RequestBody List<SmsHomeNewProduct> homeNewProductList, Principal principal) {
        if(principal==null){
            return CommonResult.unauthorized(null);
        }
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        int count = homeNewProductService.create(homeNewProductList,umsAdmin.getId());
        if (count > 0) {
    ...
    public CommonResult<CommonPage<SmsHomeNewProduct>> list(@RequestParam(value = "productName", required = false) String productName,
        ...
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        List<SmsHomeNewProduct> homeNewProductList = homeNewProductService.list(umsAdmin.getId(),productName, recommendStatus, pageSize, pageNum);
