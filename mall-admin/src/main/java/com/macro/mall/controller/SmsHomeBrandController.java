...
public class SmsHomeBrandController {
    ...
    @ResponseBody
    public CommonResult create(@RequestBody List<SmsHomeBrand> homeBrandList, Principal principal) {
        if(principal==null){
            return CommonResult.unauthorized(null);
        }
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        int count = homeBrandService.create(homeBrandList,umsAdmin.getId());
        if (count > 0) {
    ...
    public CommonResult<CommonPage<SmsHomeBrand>> list(@RequestParam(value = "brandName", required = false) String brandName,
        ...
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        List<SmsHomeBrand> homeBrandList = homeBrandService.list(umsAdmin.getId(),brandName, recommendStatus, pageSize, pageNum);
