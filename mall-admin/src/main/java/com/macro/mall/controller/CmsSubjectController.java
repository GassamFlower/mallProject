...
public class CmsSubjectController {
    ...
    @ResponseBody
    public CommonResult<List<CmsSubject>> listAll(Principal principal) {
        if(principal==null){
            return CommonResult.unauthorized(null);
        }
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        List<CmsSubject> subjectList = subjectService.listAll(umsAdmin.getId());
    ...
    public CommonResult<CommonPage<CmsSubject>> getList(@RequestParam(value = "keyword", required = false) String keyword,
        ...
        String username = principal.getName();
        UmsAdmin umsAdmin = adminService.getAdminByUsername(username);
        List<CmsSubject> subjectList = subjectService.list(umsAdmin.getId(),keyword, pageNum, pageSize);
